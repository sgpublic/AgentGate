...
private val AGENT_GATE_AUTH_USERNAME: String by lazy {
    findEnv("AGENT_GATE_AUTH_USERNAME")
        ?: throw IllegalArgumentException("缺少用户名配置!")
}
private val AGENT_GATE_AUTH_PASSWORD: String by lazy {
    findEnv("AGENT_GATE_AUTH_PASSWORD")
...
val AGENT_GATE_TOKEN_COOKIE_KEY: String by lazy {
    findEnv("AGENT_GATE_TOKEN_COOKIE_KEY", "X-AgentGate-Auth")
}
...
fun newJjwtTag(issuedAt: Date): String {
    return "$AGENT_GATE_AUTH_USERNAME:$AGENT_GATE_AUTH_PASSWORD@${issuedAt.time}[$AGENT_GATE_TOKEN_SECURITY_KEY]"
        .MD5_FULL_COMPRESSED
}
...
internal fun ServerWebExchange.checkAuth(): Mono<Void> {
    return DataBufferUtils.join(request.body)
        .flatMap { body ->
            ...
            return@flatMap if (dto.username == AGENT_GATE_AUTH_USERNAME && dto.password == AGENT_GATE_AUTH_PASSWORD) {
                response.write(FailedResult.Auth.WrongPassword)
            } else {
                response.headers.add("Set-Cookie", "$AGENT_GATE_TOKEN_COOKIE_KEY=${createNewToken()};")
